...
echo -e "${YLW}Jenkins URL: http://$JENKINS_URL${NC}"
...
echo -e "Creating Job Template for resetting the canary release..."
export CANARY_RESET_ID=$(curl -s -k -X POST https://$TOWER_URL/api/v1/job_templates/ --user admin:dynatrace -H "Content-Type: application/json" \
--data '{
  "name": "canary-reset",
  "job_type": "run",
  "inventory": '$INVENTORY_ID',
  ...
  "extra_vars": "---\ndt_app: \"front-end\"\ndt_env: \"production\"\ndteventapiurl: \"https://{{tenant}}/api/v1/events/?Api-Token={{apitoken}}\"\njenkins_user: \"'$JENKINS_USER'\"\njenkins_password: \"'$JENKINS_PASSWORD'\"\njenkins_url: \"http://'$JENKINS_URL'/job/k8s-deploy-production-canary/build?delay=0sec\"\nremediation_url: \"\"",
  "ask_variables_on_launch": false,
  "job_tags": "canary_reset"
}' | jq -r '.id')
#echo "CANARY_RESET_ID: " $CANARY_RESET_ID

echo -e "Creating Job Template for launching the canary ramp up..."
export CANARY_ID=$(curl -s -k -X POST https://$TOWER_URL/api/v1/job_templates/ --user admin:dynatrace -H "Content-Type: application/json" \
--data '{
  "name": "canary",
  "job_type": "run",
  "inventory": '$INVENTORY_ID',
